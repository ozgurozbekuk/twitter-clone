...
function SingleTwit(props) {

    function contentSplit(cont) {
       let newCont = cont.split(" ");
       let newsplCont = newCont.map((word,index) => {
        if(word.startsWith("#")) {
            return <span className="text-blue-400" key={index}>{word} </span>
           }else {
            return word + " "
           }
       } )
